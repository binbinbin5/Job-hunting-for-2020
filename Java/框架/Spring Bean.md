...
1. 设置对象属性(setter等)（依赖注入）
1. 如果Bean实现了BeanNameAware接口，工厂调用Bean的setBeanName()方法传递Bean的ID。（和下面的一条均属于检查Aware接口）
1. 如果Bean实现了BeanFactoryAware接口，工厂调用setBeanFactory()方法传入工厂自身
1. 将Bean实例传递给Bean的前置处理器的postProcessBeforeInitialization(Object bean, String beanname)方法
1. 调用Bean的初始化方法
1. 将Bean实例传递给Bean的后置处理器的postProcessAfterInitialization(Object bean, String beanname)方法
1. 使用Bean
1. 容器关闭之前，调用Bean的销毁方法


==Spring Bean 有五个作用域，其中最基础的有下面两种：==

- Singleton(单例)，这是 Spring 的默认作用域，也就是为每个 IOC 容器创建唯一的一个 Bean 实例。
- Prototype（原型），针对每个 getBean 请求，容器都会单独创建一个 Bean 实例。
